...
def feat_names():
    ...
    # basic global info
    res =  ['track_id','loudness','tempo','time_signature','key','mode','duration','hotnesss']
    ...
    for k in range(1,13):
        res.append( 'max_segment_tim    bre'+str(k))
    # done
    return res
...
if __name__ == '__main__':
    ...
    top50mbtags_names = [x[0] for x in top50mbtags]
    for g in GENRES:
    ...
    for genre in GENRES:
        ...
        res = conn_at.execute(q)
        artists = [x[0] for x in res.fetchall()]
        for a in artists:
            q = "SELECT mbtag FROM artist_mbtag WHERE artist_id='"+a+"'"
            res = conn_at.execute(q)
            mbtags = [x[0] for x in res.fetchall()]
            artist_is_safe = True
    ...
    for g in list(genre_artists.keys()):
    ...
    for genre in GENRES:
        ...
        for artist in artists:
            ...
            res = conn_tm.execute(q)
            track_ids = [x[0] for x in res.fetchall()]
            for path in [path_from_trackid(msddir,x) for x in track_ids]:
